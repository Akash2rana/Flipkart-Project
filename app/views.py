...
def showpricerange(req):
    if req.method=="GET":
        ...
        if r1 is not None and r2 is not None and r1.isdigit() and r2.isdigit():
            ...
            allproducts = Product.productmanager.all()
            context = {'allproducts':allproducts}
            return render(req,'index.html',context) 
...
def searchproduct(req):
    ...
    return render(req,'index.htmld',context)
...
def updatequantity(req,qv,productid):
...
def removecart(req,productid):
    user = req.user
    cartitems=Cart.objects.get(productid=productid)
    cartitems.delete()
    return redirect('/showcarts')
...
def payment(req):
    if req.user.is_authenticated:
        ...
        for x in  allcarts:
            ...
            orderdata=Orders.objects.create(orderid=orderid,productid=x.productid,userid=x.userid,qty=x.qty)
            orderdata.save()
